...
class Agent:
    ...
    def add(self, obs, action, reward, done, fail, next_obs, mean, std, env_idx=0):
        self.replay_buffer.add(obs, action, reward, done, fail, next_obs, mean, std, env_idx)

    def train(self):
        self.replay_buffer.update(self.policy, self.value)
        ...
        for _ in range(self.n_epochs):
            indices = np.random.permutation(len(self.replay_buffer.states_buffer))
            start_idx = 0
            end_idx = self.batch_size

            while end_idx <= len(indices):
                batch_indices = indices[start_idx:end_idx]
                trajs = self.replay_buffer.sample(batch_indices)
    ...
    def update(self, trajs):
        states, actions, targets, gaes, old_log_probs, mu_log_probs = trajs
        ...
        mu_log_probs_tensor = torch.tensor(mu_log_probs, device=self.device, dtype=torch.float32)
        ...
        ratios = torch.exp(log_probs - mu_log_probs_tensor)
