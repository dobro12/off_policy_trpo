...
def getPaser():
    ...
    parser.add_argument('--name', type=str, default='PPO', help='save name.')
    ...
    parser.add_argument('--n_envs', type=int, default=8, help='gym environment name.')
    parser.add_argument('--n_steps', type=int, default=8000, help='# of steps per update.')
    # for networks
    ...
    parser.add_argument('--ent_coeff', type=float, default=0.0, help='entropy coefficient for loss.')
    parser.add_argument('--max_kl', type=float, default=0.01, help='maximum kl divergence.')
    parser.add_argument('--clip_value', type=float, default=0.05, help='PPO\'s clip value.')
...
def train(args):
    ...
    # for log
    p_loss_logger = Logger(args.save_dir, 'p_loss')
    ...
    while total_step < args.total_steps:
        ...
        while step < args.n_steps:
            ...
            for env_idx in range(args.n_envs):
                ...
                agent.add(observations[env_idx], actions[env_idx], rewards[env_idx], dones[env_idx], fail, next_observation, means[env_idx], stds[env_idx], env_idx)
        ...
        v_loss, p_loss, kl, entropy = agent.train()
        p_loss_logger.write([step, p_loss])
        ...
        log_data = {
            ...
            "train/p_loss":p_loss_logger.get_avg(), 
